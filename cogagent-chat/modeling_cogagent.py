...
def _make_causal_mask(
        input_ids_shape: Tuple[int], dtype: mx.Dtype, past_key_values_length: int = 0
):
    """
    Make causal mask used for bi-directional self-attention.
    """
    bsz, tgt_len = input_ids_shape
    mask = mx.full((tgt_len, tgt_len), np.finfo(dtype).min)
    mask_cond = mx.arange(mask.shape[-1])
    # TODO
    bool_mask = mask_cond < (mask_cond + 1).reshape(mask.shape[-1], 1)
    mask[bool_mask] = 0
    mask = mask.to(dtype)

    if past_key_values_length > 0:
        mask = mx.concatenate([mx.zeros(tgt_len, past_key_values_length, dtype=dtype), mask], dim=-1)
...
def _expand_mask(mask: mx.array, dtype: mx.Dtype, tgt_len: Optional[int] = None):
    ...
    expanded_mask = mx.broadcast_to(mask[:, None, None, :], (bsz, 1, tgt_len, src_len)).to(dtype)

    inverted_mask = 1.0 - expanded_mask
    inverted_mask[inverted_mask.to(mx.bool_)] = np.finfo(dtype).min
    return inverted_mask

def get_expert_mask(token_type_ids: mx.array) -> Tuple[mx.array, mx.array]:
    # (B, L) -> (B, L), (B, L)
    vision_token_mask = mx.zeros_like(token_type_ids, dtype=mx.bool_)
...
class VisionExpertMLP(nn.Module):
    ...
    def forward(self, hidden_states: "mx.array(B, L, D)", token_type_ids: "mx.array(B, L)"):
        output = mx.zeros(hidden_states.shape, dtype=hidden_states.dtype, device=hidden_states.device)
...
class CogAgentDecoderLayer(nn.Module):
    def __init__(self, config):
        ...
        self.post_cross_attention_layernorm = nn.RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
